# Codex Notes

Quick pointers to keep changes safe and avoid common pitfalls in this repo.

## Architecture & Routing
- Frontend is Vite + React Router (lazy-loaded routes in `src/App.tsx`). New pages must be added there or they will 404.
- Blog posts are data-driven from `src/data/blogPosts.ts`; some posts render custom components via the `content` key (`front-4-veneers`, `single-tooth-veneers`).
- `/veneers/` is the single veneers pillar. Children: `/veneers/cost-los-angeles` (cost guide, `VeneersCostGuideLosAngeles.tsx`), `/veneers/front-teeth-veneers-los-angeles` (2–4 teeth hub, `FrontTeethVeneers.tsx`), `/veneers/2-front-teeth-veneers-cost-los-angeles` (long-tail child that links up to the hub), and `/veneers/1-tooth-veneer-los-angeles`. `/veneers-los-angeles` is retired (page deleted, 301 → `/veneers/`). Internal linking widgets prefer these URLs over legacy cost blogs, and no in-repo link should point at the second domain `exquisiteveneersla.com`.

## Redirects & Fallbacks
- Production redirects live in `vercel.json`; this is the source of truth for the Vercel-hosted site. Keep `scripts/redirect-tests` (legacy list + canonical map) aligned whenever adding or changing mappings.
- Run redirect tests through Vercel routing: `npx vercel dev --listen 127.0.0.1:8899 --yes`, then `npm run test:redirects`. Plain Vite does not apply `vercel.json`.
- Legacy Netlify files remain for historical/static-host context only.
- Assets must **not** be caught by wildcard redirects. `/assets/*` is explicitly forced to 200 to prevent lazy-loaded chunks (e.g., `Veneers-*.js`) from being redirected to `/veneers/`.
- Static fallbacks can be generated manually via `npm run generate:fallbacks` when intentionally refreshing legacy fallback HTML. The main production build now relies on `prerender:static` for crawlable route snapshots in `dist/`.

## SEO & Sitemap
- Structured data helpers live under `src/components/seo/` and `src/utils/sitemapGenerator.ts`. Add new high-value pages to the static list in `generateSitemapData`.
- `public/sitemap.xml` is generated by `npm run build` (via `scripts/generate-sitemap.js`). Update after adding routes or changing publish flags.

## Testing & Commands
- Primary gates: `npm run lint`, `npm run typecheck`, `npm run build`. `npm run build:prod` is retained as an alias for older workflows.
- **`npm run build` does not typecheck.** Vite strips types via esbuild and never invokes `tsc`, so a green build proves nothing about type correctness — several broken interfaces and impossible narrowings reached `main` this way before `npm run typecheck` existed. `.github/workflows/ci.yml` now runs it on every PR; run it locally too.
- Listeners attached to `document`/`window` from a `useEffect` must be **defined inside that effect**. Handlers defined in the component body get a fresh identity every render, so cleanup calls `removeEventListener` with a function that was never registered and the original stays attached forever, holding whatever state it captured. `src/components/ui/comparison-slider.tsx` had exactly this bug: the before/after slider kept following the cursor after mouseup and leaked one live listener per drag.
- Redirect verification: `node scripts/test-redirects.mjs` (set `REDIRECT_TEST_BASE` when testing preview/production instead of local Vercel dev).
- Browser smoke: `node test-browser.js` against a running server (`npm run dev` or `npm run preview`).
- Cherry responsive regression: `npx playwright test src/__tests__/cherry-widget.mobile.spec.ts --project=chromium --project=webkit --workers=2 --retries=0`.

## Hero Media & Floating UI
- Hero background videos go through one shared chain: `VideoHero` → `Desktop/MobileVideoHero` → `VideoBackground` (~23 pages, no inline hero videos). They lazy-load via IntersectionObserver + `requestIdleCallback`.
- Every video-backed hero uses the shared abstract black-and-gold poster at `/public/lovable-uploads/exquisite-black-gold-hero.png`. It stays visible during deferred loading and player initialization, then crossfades only after playback starts. The same asset remains visible for reduced motion, constrained connections, `disableVideo`, and `preferStaticOnMobile`. Keep testimonial and other contained-player thumbnails route-specific.
- The Cherry launcher always shows the complete `Pay over time` / `No hard credit checks • 0% APR options` compact copy. It is 288px normally and 232px at 320px, where the supporting line wraps. Reserved left space clears the Concierge. The homepage `FloatingActionButton` that used to stack above the pill was removed, so the pill is now the only bottom-right fixed control; if another one is ever added, it must clear the pill rather than overlap it. Do not restore icon-only, clipping, ellipsis, or overlapping fixed controls. Overlaying scrolling body copy is expected.
- Cherry chip alignment (2026-09-07): keep the title and every wrapped supporting line left-aligned in one text column. `CherryWidgetProvider` overrides vendor styles with automatic button height, 10px vertical / 14px horizontal padding, a 10px content gap, and a 2px text gap. Center the icon vertically; never restore the vendor fixed height or centered subtext. Verified with the live Cherry runtime at desktop/mobile sizes and all 18 Chromium/WebKit Cherry checks; typecheck/build pass, lint has 0 errors and 18 existing warnings.
- Safari: do not hide the launcher from `window.scrollY` alone. Measure visualViewport-aware scroll, hide only at the true top (≤16px), and keep it visible through toolbar-collapse jitter after the 96px reveal. Keep `transform: none` on the fixed mount so Safari cannot re-sticky or clip it.

## Content Notes
- Legacy front-teeth cost blogs are unpublished and 301 to the new hub. Do not republish without revisiting redirects and sitemap. Retired generated posts are kept unpublished via `UNPUBLISHED_SLUGS` in `scripts/generate-blog-posts.mjs` (regeneration-safe); never flip `published` by hand-editing `generatedBlogPosts.ts`.
- Internal linking (`src/components/InternalLinkingWidget.tsx`) is context-aware; update priorities when adding new veneers content to keep the hub surfaced.
- Cherry financing copy is centralized in `src/constants/cherry.ts`; follow `docs/cherry-credit-disclosure.md` and avoid unqualified credit-check wording outside the approved "no hard credit check" badge.
- Veneer **market** cost ranges live in `src/constants/veneerCosts.ts` under the same `VERIFY-BEFORE-PUBLIC` convention as Cherry. `/veneers/cost-los-angeles` hides dollar figures ("confirmed at your consultation") until the practice signs off and `VENEER_COST_VERIFIED = true`. Open follow-ups (cost verification) are tracked in `CLAUDE.md` → Open follow-ups. Social `og:image` values must point at files that exist in `public/lovable-uploads/`; the missing `dc09fcc1` asset was replaced with `DEFAULT_OG_IMAGE`. The `exquisiteveneersla.com` second domain is not used for this site — never link to it.
- Practice address: wherever `ADDRESS` (`src/constants/contact.ts`) is shown in the UI, render `<OpenInMapsButton source="…" />` beside it (opens `GOOGLE_MAPS_SHORT_URL` in a new tab, `target="_blank" rel="noopener noreferrer"`, with `trackContactMethodClick({ method: 'directions' })`). `Footer` and `PracticeLocationSection` already have their own maps buttons. Skip JSON-LD / `*StructuredData` / schema occurrences — those are not visible UI.
