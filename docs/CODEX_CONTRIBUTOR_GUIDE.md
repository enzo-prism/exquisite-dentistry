...
- **Global analytics**: App-wide providers live in `src/App.tsx`. Vercel Analytics and Vercel Speed Insights are both mounted there with `@vercel/analytics/react` and `@vercel/speed-insights/react`; keep them singleton-scoped so React Router navigations are tracked once per pageview and client performance beacons fire once per route visit.
