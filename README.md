...
Use Node.js 20 or newer (CI uses 22) with npm installed - [install with nvm](https://github.com/nvm-sh/nvm#installing-and-updating)
...
Copy `.env.example` to `.env` if HTML-based Google Search Console verification is needed. When `VITE_GSC_VERIFICATION` is empty, Vite omits the verification meta tag instead of shipping a placeholder. The Vite dev server picks up changes automatically, but rebuild before deploying after adding or changing a token.
...
- **Spam + analytics guards** – a lightweight honeypot field (`bot-field`) short circuits obvious spam while still returning a “success” UI. `trackFormSubmission('contact_form')` fires only after the form endpoint confirms a real submission and produces the canonical GA4 `generate_lead` only for non-test new-patient requests; existing-patient, vendor, benefits-only, invalid, failed, and honeypot submissions must emit no acquisition lead. See `docs/ga4-measurement-plan.md` before changing tracking.
