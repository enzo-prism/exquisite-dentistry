...
- **Vercel Analytics** – visitor/pageview tracking is mounted once at the app shell via `@vercel/analytics/react` in `src/App.tsx`. Keep it as a singleton inside the top-level router tree so SPA route changes are captured. After shipping to production, open a few routes on the live site and confirm data appears in the Vercel Analytics dashboard (blockers/content filters can delay or suppress events).
