...
- **Vercel Analytics & Speed Insights** – visitor/pageview tracking and Real Experience Score collection are both mounted once at the app shell in `src/App.tsx` via `@vercel/analytics/react` and `@vercel/speed-insights/react`. Keep both as singletons inside the top-level router tree so SPA route changes are captured correctly. After shipping to production, open a few live routes on desktop and mobile, then confirm Analytics pageviews and Speed Insights data start appearing in Vercel (content blockers can delay or suppress collection).
