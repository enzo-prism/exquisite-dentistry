import optimizedImageManifest from './optimizedImageManifest.json';

// Generated by scripts/validate-images.js: optimized base path -> size suffixes on disk.
const OPTIMIZED_VARIANTS: Record<string, string[]> = optimizedImageManifest;
...
const buildSrcSet = (base: string, format: 'avif' | 'webp') => {
  const availableSuffixes = OPTIMIZED_VARIANTS[base];
  if (!availableSuffixes) return undefined;

  const entries = OPTIMIZED_SIZES
    .filter(({ suffix }) => availableSuffixes.includes(suffix))
